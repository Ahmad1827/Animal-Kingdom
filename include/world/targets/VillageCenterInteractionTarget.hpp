...
private:
    ...
    std::string getJobName(sim::Job job) const {
        switch(job) {
            case sim::Job::Idle: return "Idle (Resting)";
            case sim::Job::Builder: return "Clan Builder";
            case sim::Job::Woodcutter: return "Woodcutter";
            case sim::Job::StoneGatherer: return "Quarryman";
            case sim::Job::Forage: return "Forager";
            case sim::Job::Guard: return "Settlement Guard";
            case sim::Job::Scout: return "Border Scout";
            default: return "Tribesman";
        }
    }
...
public:
    ...
    std::string getInteractionTitle() const override {
        ...
        if (!v) return "Clan Hearth";
        if (currentState == MenuState::Roster) return v->name + " - Tribal Roster";
        if (currentState == MenuState::Construction) return v->name + " - Construction Projects";
        return "Clan Center: " + v->name;
    }

    int getPriority() const override { return 20; }
    void onInteract() override { currentState = MenuState::Main; }
    ...
    std::vector<InteractionMenuEntry> buildInteractionMenu() override {
        ...
        if (currentState == MenuState::Main) {
            entries.push_back({"Population: " + std::to_string(v->members.size()) + " clan apes", nullptr});
            entries.push_back({"Stores: " + std::to_string(v->food) + " Food | " + std::to_string(v->wood) + " Wood | " + std::to_string(v->stone) + " Stone", nullptr});
            entries.push_back({"", nullptr});
            entries.push_back({"[ Manage Tribal Labor & Roles ]", [this]() { currentState = MenuState::Roster; }});
            entries.push_back({"[ Inspect Construction Queue ]", [this]() { currentState = MenuState::Construction; }});
        }
        else if (currentState == MenuState::Roster) {
            entries.push_back({"<- Return to Overview", [this]() { currentState = MenuState::Main; }});
            entries.push_back({"", nullptr});
            ...
            if (v->constructionQueue.empty()) {
                entries.push_back({"All settlement structures are fully built.", nullptr});
            } else {
                for (auto structId : v->constructionQueue) {
                    ...
                    if (sd) {
                        ...
                        entries.push_back({"Structure Project: " + std::to_string(pct) + "% Completed", nullptr});
                    }
                }
            }
        }
