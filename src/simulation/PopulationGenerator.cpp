...
EntityID PopulationGenerator::generatePlayerDynasty(SimulationRegistry& registry, uint32_t worldSeed) {
    ...
    founder.worldY = pVillage.centerY;
    ...
    spouse.age = 32.0f;
    spouse.worldX = pVillage.centerX + 50.0f;
    spouse.worldY = pVillage.centerY;
    ...
    child.age = 12.0f;
    child.worldX = pVillage.centerX - 50.0f;
    child.worldY = pVillage.centerY;
    ...
    sibling.age = 33.0f;
    sibling.worldX = pVillage.centerX + 150.0f;
    sibling.worldY = pVillage.centerY;
    ...
    for (int i = -1; i <= 1; ++i) {
        ...
        res.worldX = pVillage.centerX + (i * 800.f) + 400.f;
...
void PopulationGenerator::generateVillages(SimulationRegistry& registry, uint32_t worldSeed) {
    ...
    for (int v = 0; v < numVillages; ++v) {
        ...
        for (int i = -1; i <= 1; ++i) {
            ...
            res.id = IDGenerator::generateEntityID();
            res.type = ResourceType::Food;
            res.worldX = village.centerX + (i * 600.f) + 300.f;
            res.worldY = TerrainGenerator::getTerrainHeight(res.worldX, worldSeed) - 50.f;
            res.amount = 8;
            res.maxAmount = 8;
            res.regrowTimer = 0.f;
