...
void SimulationRegistry::registerStructure(const StructureData& structure) { structures[structure.id] = structure; }
...
StructureData* SimulationRegistry::getStructure(StructureID id) { return structures.count(id) ? &structures[id] : nullptr; }
...
std::unordered_map<StructureID, StructureData>& SimulationRegistry::getAllStructures() { return structures; }
}