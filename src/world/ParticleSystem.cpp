...
void ParticleSystem::update(float dt, const sf::FloatRect& viewBounds, const sf::Vector2f& windVector, float rain, float timeOfDay) {
    if (std::rand() % 100 < 5 + (std::abs(windVector.x) * 50)) {
    ...
    if ((timeOfDay > 0.75f || timeOfDay < 0.2f) && std::rand() % 100 < 8) {
    ...
    for (auto it = particles.begin(); it != particles.end();) {
        ...
        if (it->type == 0) {
            ...
            it->velocity.y = std::sin(it->phase * 10.f) * 20.f;
        }
